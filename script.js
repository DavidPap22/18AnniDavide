...
AFRAME.registerComponent('face-camera', {
  schema: {
    mode: { type: 'string', default: 'y' }, // 'y' o 'free'
...
// Track degli audio in riproduzione per evitare ripetizioni
const playingAudios = {}; // playingAudios[id] = Audio object or true
...
function distributeItemsCircle(radius = 2.0, height = 2.2) {
  ...
  itemIds.forEach((id, i) => {
    ...
    // usa un nome di animazione univoco per evitare conflitti
...
// ---------- PARTICLES, FUMO, LUCE ----------
// uso animation__float/animation__rise per garantire che A-Frame gestisca correttamente le animazioni
...
function setupInteractions() {
  // ora Radio è in linkMap (apre Spotify) e linkMap viene controllato PRIMA degli audio locali
  ...
  qr.addEventListener('click', async () => {
    ...
    // assicurati volume alto quando il video parte
  ...
  itemIds.forEach(id => {
    ...
    el.addEventListener('click', () => {
      // 1) se c'è un link, aprilo (non toccare bgMusic)
      if (linkMap[id]) {
        window.open(linkMap[id], '_blank');
        return;
      }
      // 2) se c'è audio locale, riproducilo - ma solo se non è già in riproduzione
      if (audioMap[id]) {
        if (playingAudios[id]) {
          // già in riproduzione -> ignora
          return;
        }
        try { bgSavedTime = bgMusic.currentTime; bgMusic.pause(); } catch (e) { }
        const a = new Audio(audioMap[id]);
        playingAudios[id] = a;
        // riproduci; se il play fallisce (promise), rimuovere flag
        const p = a.play();
        if (p && p.then) {
          p.catch(()=>{ playingAudios[id] = null; try{ bgMusic.play(); }catch(e){} });
        }
